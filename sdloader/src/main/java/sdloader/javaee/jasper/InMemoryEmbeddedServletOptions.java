...
public class InMemoryEmbeddedServletOptions implements Options {
	...
	// Logger
	private SDLoaderLog log = SDLoaderLogFactory
			.getLog(InMemoryEmbeddedServletOptions.class);
	 ...
	 */
	public InMemoryEmbeddedServletOptions(ServletConfig config,
		...
		if (senderr != null) {
			if (senderr.equalsIgnoreCase("true")) {
				...
				if (log.isWarnEnabled()) {
					log.warn(Localizer
							.getMessage("jsp.warning.sendErrToClient"));
				}
			}
		}
		...
		if (debugInfo != null) {
			if (debugInfo.equalsIgnoreCase("true")) {
				...
				if (log.isWarnEnabled()) {
					log
							.warn(Localizer
									.getMessage("jsp.warning.classDebugInfo"));
				}
			}
		}
		...
		String modificationTestInterval = config
				.getInitParameter("modificationTestInterval");
		if (modificationTestInterval != null) {
			try {
				this.modificationTestInterval = Integer
						.parseInt(modificationTestInterval);
			} catch (NumberFormatException ex) {
				if (log.isWarnEnabled()) {
					log
							.warn(Localizer
									.getMessage("jsp.warning.modificationTestInterval"));
				}
			}
		}
		...
		String errBeanClass = config
				.getInitParameter("errorOnUseBeanInvalidClassAttribute");
		...
		String displaySourceFragment = config
				.getInitParameter("displaySourceFragment");
		if (displaySourceFragment != null) {
			if (displaySourceFragment.equalsIgnoreCase("true")) {
				...
				if (log.isWarnEnabled()) {
					log.warn(Localizer
							.getMessage("jsp.warning.displaySourceFragment"));
				}
			}
		}
		...
		// web-application.
		tldLocationsCache = new InMemoryTldLocationsCache(context);
